...
Поэтому новый модуль создаётся сразу в целевом дереве `src/mcp_russia/`.
...
- При добавлении модуля обновляйте не только код, но и пользовательские тексты, ресурсы, промпты и тестовые ожидания.
