# Сценарий: Журналист — Подготовка материалов на основе данных

> Как использовать `mcp-russia` для подготовки, обогащения и проверки журналистских материалов на основе официальных и публичных API. Все источники, формулировки и примеры приведены для работы с российскими государственными данными.

---

## Ключевое преимущество

В отличие от сценария [журналист-расследователь](./zhurnalist-rassledovatel.md), здесь фокус не на выявлении нарушений, а на **повседневной журналистике, основанной на данных**: быстро собрать проверяемые цифры, сравнить регионы или муниципалитеты и получить основу для публикации без ручного обхода десятка порталов.

---

## Материал 1: «Сколько стоит депутат?»

### Тема

Сравнить представительские и аппаратные расходы парламентариев: кто тратит больше всего, кто экономит и какие категории расходов доминируют.

### Расследование

**1. Рейтинг расходов**

> Промпт: «Выведи 10 депутатов Госдумы с наибольшими расходами по компенсациям за 2024 год»

```
API: gosduma_info_deputata (vypolnit_paket для топ-10) *(расходы депутатов — планируемый инструмент)*
```

**2. Сравнение по фракциям**

> Промпт: «Какова средняя сумма расходов аппарата по фракциям? Какая фракция тратит больше всего в пересчёте на одного депутата?»

```
API: gosduma_spisok_deputatov (все) + gosduma_info_deputata (пакетно) *(расходы депутатов — планируемый инструмент)*
```

**3. Детализация по категориям**

> Промпт: «Сколько Госдума тратит суммарно на авиаперелёты, питание и ГСМ?»

LLM агрегирует данные и формирует:

```
РАСХОДЫ ГОСДУМЫ — КОМПЕНСАЦИИ ДЕПУТАТАМ 2024

Общие расходы 450 депутатов: ₽ 12 450 000 000

| Категория                | Итого         | Среднее/деп. |
|--------------------------|---------------|-------------|
| Публикации и СМИ         | ₽ 3,8 млрд   | ₽ 8 444 000 |
| Авиаперелёты             | ₽ 2,9 млрд   | ₽ 6 444 000 |
| ГСМ                      | ₽ 1,7 млрд   | ₽ 3 777 000 |
| Питание                  | ₽ 1,2 млрд   | ₽ 2 666 000 |
| Аренда транспорта        | ₽ 0,9 млрд   | ₽ 2 000 000 |
| Консультационные услуги  | ₽ 0,8 млрд   | ₽ 1 777 000 |
| Прочее                   | ₽ 1,1 млрд   | ₽ 2 444 000 |

Депутаты, НЕ использовавшие компенсации: 12 из 450
Депутаты, использовавшие > 90% лимита: 63 из 450
```

### Лид материала

> «450 депутатов Госдумы израсходовали ₽ 12,45 млрд на компенсации в 2024 году — в среднем ₽ 27,7 млн на каждого. Авиаперелёты и ГСМ составляют 37 % от общей суммы. Депутат [Фамилия] возглавил рейтинг с ₽ [сумма], тогда как [Фамилия] оказался наиболее экономным — ₽ [сумма].»

**Каждая цифра верифицируема через открытые данные Госдумы.**

---

## Материал 2: «Карта неравенства: здравоохранение по регионам»

### Тема

Сравнить инфраструктуру здравоохранения между субъектами РФ: коечный фонд, кадры, расходы и итоговые показатели.

### Расследование

**1. Инфраструктура по регионам**

> Промпт: «Сравни количество больничных коек на 1000 жителей во всех субъектах РФ»

```
API: minzdrav_poisk_med_organizatsiy (по субъекту) + rosstat_demografiya (численность)
```

**2. Расходы на здравоохранение**

> Промпт: «Сколько каждый субъект РФ тратит на здравоохранение в расчёте на душу населения?»

```
API: rosaudit_ispolnenie_byudzheta + rosstat_demografiya (численность)
```

**3. Итоговые показатели**

> Промпт: «Какова ожидаемая продолжительность жизни в каждом субъекте РФ? Каков уровень младенческой смертности?»

```
API: rosstat_demografiya
```

### Инфографика

```
КАРТА ОБЩЕСТВЕННОГО ЗДРАВООХРАНЕНИЯ — 2024

                Коек/1000 жит.   Расходы/душа   Ожид. продолж.
  Москва  ████████░     4,1          ₽ 28 500      78,1 года
  СПб   ███████░░     3,6          ₽ 24 200      77,3 года
  Татарстан ██████░     2,9          ₽ 19 800      76,5 года
  ...
  Тыва    ███░░░░░░     1,4          ₽ 9 200       69,1 года
  Ингушетия ██░░░░░░░     1,2          ₽ 7 800       70,3 года

  Разница Москва vs Тыва: 2,9x койки | 3,1x расходы | 9 лет жизни
```

---

## Материал 3: «Ключевая ставка: как рост влияет на кошелёк россиян»

### Тема

Показать, как изменение ключевой ставки и инфляции влияет на повседневные расходы домохозяйств и стоимость кредита.

### Расследование

**1. Исторический ряд ключевой ставки**

> Промпт: «Покажи динамику ключевой ставки за последние 5 лет и сравни с инфляцией»

```
API: cbrf_sravnit_valyuty(kody=["USD", "EUR", "CNY"])
```

**2. Влияние на кредитование**

> Промпт: «Какова стоимость ипотеки на ₽ 10 млн при текущей ключевой ставке по сравнению со ставкой 2 года назад?»

```
API: rosstat_indikator_dannye(kod="...") *(ряды по кредитованию — планируемый источник)*
```

**3. Международное сравнение**

> Промпт: «Какова реальная процентная ставка в России по сравнению с другими странами?»

LLM рассчитывает: ключевая ставка (21,00 %) − инфляция (9,50 %) = **реальная ставка 11,50 %**

```
РЕАЛЬНЫЕ ПРОЦЕНТНЫЕ СТАВКИ — МЕЖДУНАРОДНОЕ СРАВНЕНИЕ

  🇷🇺 Россия     ████████████████████░  11,50 %
  🇹🇷 Турция     ███████████████░░░░░░   8,2 %
  🇲🇽 Мексика    █████████░░░░░░░░░░░░   5,2 %
  🇮🇳 Индия      ███████░░░░░░░░░░░░░░   3,1 %
  🇺🇸 США        ████░░░░░░░░░░░░░░░░░   1,8 %
  🇪🇺 Еврозона   ███░░░░░░░░░░░░░░░░░░   1,2 %
  🇯🇵 Япония     █░░░░░░░░░░░░░░░░░░░░  -0,1 %
```

### Лид материала

> «При ключевой ставке 21,00 % и инфляции 9,50 % реальная ставка достигает 11,50 %. Ипотечный кредит на ₽ 10 млн, который в 2021 году обходился примерно в ₽ 42 000 в месяц, теперь стоит около ₽ 175 000. Источники: данные Банка России и официальной статистики.»

---

## Материал 4: «Рекордные пожары: цифры Росгидромета и Росприроднадзора»

### Тема

Освещать сезон природных пожаров и связанных климатических рисков на основе официальных данных мониторинга.

### Расследование

**1. Очаги природных пожаров**

> Промпт: «Сколько очагов природных пожаров зарегистрировано в Сибирском федеральном округе в 2024 году? Сравни с предыдущими 5 годами»

```
API: rosgidromet_preduprezhdeniya(subiekt="sibirskiy_fo")
```

**2. Вырубка лесов**

> Промпт: «Каков объём обезлесения в Сибирском ФО за последние 12 месяцев?»

```
API: rosprirodnadzor_poisk_proverok(vid_nadzora="лесной надзор") *(региональная фильтрация — планируемое расширение)*
```

**3. Водные ресурсы**

> Промпт: «Каков уровень водохранилищ в регионах с наибольшим числом пожаров?»

```
API: rosvodresursy_info_vodokhranilishcha
```

**4. Официальные публикации**

> Промпт: «Найди официальные публикации о введении режима ЧС из-за природных пожаров в 2024 году»

```
API: publikatsii_poisk_aktov(tekst="режим ЧС природные пожары 2024")
```

---

## Материал 5: «Выборы 2026: деньги предвыборной кампании»

### Тема

Отслеживать финансирование предвыборной активности и ранние финансовые сигналы кампаний.

### Расследование

> Промпт: «Выведи кандидатов на пост губернатора Московской области с зарегистрированными финансовыми отчётами в ЦИК»

```
API: cekrf_poisk_kandidata(fio="...", god=2026)
      cekrf_kandidat_podrobno (для каждого кандидата)
```

> Промпт: «Кто крупнейшие спонсоры губернаторских кампаний по всей России?»

```
API: cekrf_poisk_kandidata + cekrf_kandidat_podrobno (пакетно)
```

---

## Советы для журналистов

### 1. Всегда указывай источник

```
«По данным Банка России (серия ЦБ РФ, запрос от 23.03.2025),
ключевая ставка составляет…»
```

### 2. Используй `vypolnit_paket` для сравнений

Когда материал требует сравнения нескольких регионов или муниципалитетов, один пакетный запрос решает задачу:

```json
[
  {"imya_instrumenta": "rosstat_spisok_regionov", "argumenty": {}},
  {"imya_instrumenta": "rosstat_indikator_dannye", "argumenty": {"kod": "XXX", "subiekt": "Москва"}},
  {"imya_instrumenta": "rosstat_indikator_dannye", "argumenty": {"kod": "XXX", "subiekt": "Санкт-Петербург"}}
]
```

### 3. Используй `splanirovat_zapros` для сложных материалов

> Промпт: «Мне нужно подготовить материал о влиянии бюджетных трансфертов на муниципалитеты Дальнего Востока. Спланируй запросы»

Инструмент возвращает полный маршрут расследования.

### 4. Данные + автор-составитель = готовый материал

Комбинируй инструменты работы с данными с агентом-автором-составителем, если нужен черновик аналитической записки, справки, пояснительного текста или структурированного приложения к публикации.

Сценарий `mcp-russia` работает с российскими государственными данными и API: все источники, формулировки и примеры приведены для работы с российским контекстом.

---

_Источники: Банк России, Росстат, парламентские и электоральные API, портал bus.gov.ru (Федеральное казначейство), экологические и отраслевые источники, официальные публикации и другие интеграции._
