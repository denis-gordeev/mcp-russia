...
> Как в `mcp-russia` собрать единый профиль парламентария: голосования, инициативы, расходы, трансферты и связанные источники в одном отчёте. Некоторые инструменты (отмечены как «планируемый») относятся к модулям, которые ещё не реализованы.
...
- `kaznacheistvo_dannye(avtor="[ФИО]", god=2024)` *(планируемый модуль «Федеральное казначейство»)*
...
В текущем состоянии репозитория такой отчёт стоит понимать как публичный workflow `mcp-russia` над набором парламентских и избирательных интеграций. Реализованные модули (`gosduma`, `cekrf`, `zakupki`, `kad_arbitrazh`) полностью функциональны; инструменты, отмеченные как «планируемый», будут добавлены в будущих модулях.

---

## Для Совета Федерации

> ⚠️ Модуль `sovfed` планируется к реализации в будущих релизах. Указанные ниже инструменты пока недоступны.

Аналогичный отчёт работает для сенаторов, с заменой инструментов:

| Данные | Дума | Совет Федерации |
|--------|------|-----------------|
| Профиль | `gosduma_info_deputata` | `sovfed_info_senatora` *(планируемый модуль)* |
| Голосования | `gosduma_golosovaniya` | `sovfed_golosovaniya` *(планируемый модуль)* |
| Инициативы | `gosduma_zakonoproekty` | `sovfed_zakonoproekty` *(планируемый модуль)* |
| Расходы | `gosduma_raskhody_deputata` *(планируемый)* | (через портал открытых данных) |
| Комиссии | `gosduma_spisok_komitetov` | `sovfed_spisok_komissiy` *(планируемый модуль)* |

Модуль Совета Федерации планируется с подробным охватом законопроектов, поправок, повестки заседаний и состава комиссий.
...
6 запросов параллельно — профиль + расходы 3 депутатов за один вызов. `gosduma_raskhody_deputata` — планируемый инструмент.
