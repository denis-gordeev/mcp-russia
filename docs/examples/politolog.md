...
> Публичный сценарий для `mcp-russia`: как исследователи политических институтов могут изучать поимённые голосования, коалиции, дисциплину фракций, распределение трансфертов и электоральное финансирование на основе российских API.
...
| Источник | Модуль | Инструменты |
|------|---------|-----------|
| **Государственная Дума** | `gosduma` | Депутаты, законопроекты, фракции, комитеты |
| **ЦИК РФ** | `cekrf` | Электоральное финансирование, кандидаты, результаты выборов |
| **Росстат** | `rosstat` | Региональные показатели, демография, ВРП |
| **ЦБ РФ** | `cbrf` | Курсы валют, макроиндикаторы |
| **ЕИС закупки** | `zakupki` | Госзакупки, контракты, поставщики |
| **Картотека арбитражных дел** | `kad_arbitrazh` | Судебные дела, акты, стороны |
| **Счётная палата** | `rosaudit` | Контрольные мероприятия, нарушения |
| **Официальные публикации** | `publikatsii` | Нормативные акты, прavo.gov.ru |
| **РосАПИ** | `rosapi` | Поиск организаций по ИНН/ОГРН, адреса |
...
```
СРОКИ ПРОХОЖДЕНИЯ — ГОСДУМА 2024

Источник законопроекта         Медиана (дней)   Максимум
─────────────────────────────────────────────────────────
Правительство (срочный)            30               90
Правительство (обычный)           120              360
Депутат из большинства            150              540
Депутат из оппозиции              270            900+
Совет Федерации (повторное)        60              240
─────────────────────────────────────────────────────────

Вывод: Правительственные законопроекты в срочном порядке
рассматриваются в 9 раз быстрее, чем проекты оппозиционных депутатов.
```
...
```
ЗАКОНОДАТЕЛЬНАЯ ВОРОНКА — ГОСДУМА 2024
...
### `splanirovat_zapros` — Ассистированное проектирование исследования
...
### `vypolnit_paket` — Масштабный сбор данных
...
До 10 запросов в пакет, несколько пакетов последовательно = полное покрытие.

### `rekomendovat_instrumenty` — Обнаружение данных
...
| Данные организации | РосАПИ | `rosapi_poisk_org_po_inn` |
...
| Водные ресурсы | Росводресурсы | `rosvodresursy_info_vodnogo_obekta` |

---

## Методологические ограничения и предостережения
...
_Источники: Госдума, ЦИК РФ, Росстат, ЦБ РФ, ЕИС закупки, Картотека арбитражных дел, Счётная палата, Минздрав, РосАПИ, официальные публикации._

_Примечание: коэффициенты и результаты в примере иллюстративные. Для реального исследования запускайте инструменты на живых данных._
