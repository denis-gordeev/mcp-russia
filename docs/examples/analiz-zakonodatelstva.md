...
| **Совет Федерации** | `sovfed` *(планируемый)* | Законопроекты, голосования, докладчики, поправки |
...
Инструменты:
- `sovfed_search_bills(type="ФЗ", topic="искусственный интеллект")` *(планируемый модуль «Совет Федерации»)*
- `sovfed_bill_details(code=...)` *(планируемый)* — докладчик, комитет
- `sovfed_amendments(code=...)` *(планируемый)* — внесённые поправки
- `sovfed_bill_progress(code=...)` *(планируемый)* — история в Совете Федерации
...
Инструменты:
- `sovfed_bill_votes(code=...)` *(планируемый)* — результат
- `sovfed_vote_details(session_code=...)` *(планируемый)* — именной голос
...
Инструменты:
- `publikatsii_poisk_aktov(term="искусственный интеллект", type="закон")`
...
Рассмотрение в СовФеде (СовФед)         Кто финансировал? (ЦИК)
    ↓                                      ↓
Голосование в СовФеде (СовФед)        Есть ли конфликты интересов? (Росавдит)
    ↓
...
- `sovfed_search_bills(subject="цифровые платформы")` *(планируемый)*

> Промпт: «Какие законопроекты о налоговой реформе поставлены на голосование на этой неделе?»

Инструменты:
- `sovfed_agenda` *(планируемый)* — повестка Совета Федерации
...
- `sovfed_bill_text(code=...)` *(планируемый)* — текст Совета Федерации
- `sovfed_amendments(code=...)` *(планируемый)* — поправки, изменившие текст
...
Инструменты:
- `sovfed_agenda` *(планируемый)* — повестка недели
- `sovfed_bill_votes` *(планируемый)* — история аналогичных голосований
- `sovfed_committee_composition` *(планируемый)* — кто докладчик по каждому вопросу
...
Этап 1 — Идентификация:
  ...
  └── sovfed_search_bills(number=12345, convocation=8) *(планируемый)*

Этап 2 — Детализация (параллельно):
  ├── gosduma_zakonoproekty(id=...)
  ├── gosduma_zakonoproekty(id=...)
  ├── sovfed_bill_details(code=...) *(планируемый)*
  └── sovfed_bill_progress(code=...) *(планируемый)*

Этап 3 — Голосования (параллельно):
  ├── gosduma_golosovaniya(id=...)
  └── sovfed_bill_votes(code=...) *(планируемый)*
...
| «Жертвователи голосовавших выигрывают от закона?» | ЦИК РФ + Росавдит + Закупки |
...
_Примечание: Значения и сценарии иллюстративны и демонстрируют возможности `mcp-russia`. Используйте инструменты для получения актуальных данных. Инструменты, отмеченные как «планируемый», находятся в разработке._
