...
Инструменты:
- `sovfed_poisk_zakonoproektov(type="ФЗ", topic="искусственный интеллект")` *(планируемый модуль «Совет Федерации»)*
- `sovfed_podrobnosti_zakonoproekta(code=...)` *(планируемый)* — докладчик, комитет
- `sovfed_popravki(code=...)` *(планируемый)* — внесённые поправки
- `sovfed_khod_rassmotreniya(code=...)` *(планируемый)* — история в Совете Федерации
...
Инструменты:
- `sovfed_golosovaniya_zakonoproekta(code=...)` *(планируемый)* — результат
- `sovfed_podrobnosti_golosovaniya(session_code=...)` *(планируемый)* — именной голос
...
- `sovfed_poisk_zakonoproektov(subject="цифровые платформы")` *(планируемый)*
...
- `sovfed_tekst_zakonoproekta(code=...)` *(планируемый)* — текст Совета Федерации
- `sovfed_popravki(code=...)` *(планируемый)* — поправки, изменившие текст
...
Инструменты:
- `sovfed_povestka` *(планируемый)* — повестка недели
- `sovfed_golosovaniya_zakonoproekta` *(планируемый)* — история аналогичных голосований
- `sovfed_sostav_komiteta` *(планируемый)* — кто докладчик по каждому вопросу
...
Этап 2 — Детализация (параллельно):
  ...
  ├── gosduma_zakonoproekty(id=...)
  ├── sovfed_podrobnosti_zakonoproekta(code=...) *(планируемый)*
