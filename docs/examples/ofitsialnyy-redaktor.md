...
1. **Подтягивает фактические перечисления** через `kaznacheistvo_mezhbyudzhetnye_transferty(region="[Регион]")`
...
1. **Запрашивает реестр** через `minzdrav_poisk_med_organizatsiy(name="Городская больница...")`
