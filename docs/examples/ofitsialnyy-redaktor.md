...
1. **Проверяет контрагента** через `fns_info_organizatsii` / `zakupki_info_postavshchika`
...
_Источники: агент Официальный редактор (`mcp-russia`), API Банка России, Росстат, Портал открытых данных, ЕИС в сфере закупок, ГАС «Правосудие», реестр медицинских организаций._
