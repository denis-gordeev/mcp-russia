...
27 модулей · 253 инструмента · 90 ресурсов · 62 промпта
...
### `rosstat` — Федеральная служба государственной статистики (22 инструмента, 5 ресурсов, 5 промптов)

Демография, экономика, региональная статистика, федеральные округа, ВВП, ВРП, заработная плата, безработица, доходы, промышленность, бедность, пенсии, отраслевая структура ВРП по ОКВЭД 2, инвестиции по видам деятельности, региональное и окружное сравнение, поиск регионов, универсальный запрос по коду ЕМИСС. Реальные API: fedstat.ru (ЕМИСС). 92 субъекта РФ. 27 кодов показателей ЕМИСС (обновлены после перехода на ОКВЭД 2). Взвешенная агрегация по округам для относительных показателей.
...
| `poisk_regiona` | Поиск субъекта РФ по подстроке названия |
...
| `pokazateli_rosstata` | Справочник основных показателей Росстата (27 показателей, с кодами ЕМИСС) |
| `inflyatsiya` | Данные об инфляции (ИПЦ) из ЕМИСС |
| `demografiya` | Демографические данные из ЕМИСС (рождаемость, смертность, численность, естественный прирост) |
| `vrp_dannye` | Данные о валовом региональном продукте (ВРП) с разбивкой по регионам |
| `vvp_dannye` | Данные о валовом внутреннем продукте (ВВП) России |
| `zarplata_dannye` | Данные о средней заработной плате с региональной разбивкой |
| `bezrabotitsa_dannye` | Данные об уровне безработицы с региональной разбивкой |
| `dokhody_na_dushu` | Среднедушевые денежные доходы населения |
| `promyshlennoe_proizvodstvo` | Индекс промышленного производства |
| `uroven_bednosti` | Уровень бедности (доля населения с доходами ниже прожиточного минимума) |
| `srednyaya_pensiya` | Средний размер назначенных пенсий |
| `sravnenie_regionov` | Рейтинг и сравнение регионов по выбранному показателю |
| `sravnenie_okrugov` | Рейтинг федеральных округов по показателю (взвешенная агрегация для относительных) |
| `indikator_dannye` | Универсальный запрос данных показателя по коду ЕМИСС или мнемоническому коду |
| `dinamika_regiona` | Хронологический ряд регионального показателя с фильтром по годам и темпами изменения |
| `otraslevaya_struktura_vrp` | Отраслевая структура ВРП по видам экономической деятельности (ОКВЭД 2) |
| `investitsii_po_vidam` | Инвестиции в основной капитал по видам экономической деятельности |

**Ресурсы:** `data://istochniki` (источники данных), `data://metodologiya`, `data://pokazateli` (справочник показателей), `data://okved` (разделы ОКВЭД 2), `data://subiekty-rf` (субъекты РФ)

**Промпты:** `analiz_regiona`, `obzor_inflyatsii`, `analiz_vrp_regionov`, `obzor_truda`, `analiz_demografii`
...
| `info_deputata` | Карточка депутата: фракция, комитет, регион, созыв *(рекомендуется API-токен)* |
...
| `zakonoproekty` | Законопроекты с фильтрацией по статусу, автору (`avtor`) и релевантности (`sootvetstvie`) *(рекомендуется API-токен)* |
...
| Инструмент | Описание |
|------|----------|
| `konsul_adres_po_indeksu` | Найти адрес по почтовому индексу РФ *(рекомендуется API-ключ)* |
| `poisk_adresa` | Поиск адреса по свободному запросу через ФИАС *(рекомендуется API-ключ)* |
| `poisk_org_po_inn` | Найти организацию по ИНН *(рекомендуется API-ключ)* |
| `poisk_org_po_ogrn` | Найти организацию по ОГРН *(рекомендуется API-ключ)* |
| `spisok_bankov` | Справочник банков России с БИК |
| `konsul_bank_po_bik` | Информация о банке по БИК *(рекомендуется API-ключ)* |
...
**Авторизация:** опциональная (MCP_RUSSIA_DADATA_API_KEY для полного доступа)

### `zakupki` — Единая информационная система закупок (9 инструментов, 3 ресурса, 2 промпта)
...
| `plany_zakupok` | Планы закупок *(рекомендуется API-токен)* |
| `poisk_rnp` | Поиск в реестре недобросовестных поставщиков (РНП) *(рекомендуется API-токен)* |
...
### `rosgidromet` — Росгидромет / Open-Meteo (7 инструментов, 3 ресурса, 2 промпта)

Погода, прогнозы, качество воздуха, гидрология. Данные через Open-Meteo (Росгидромет — профиль ведомства).
...
### `minobrnauki` — Минобрнауки и Рособрнадзор (17 инструментов, 3 ресурса, 2 промпта)

Вузы, аккредитация, лицензии, рейтинги, гранты, контроль деятельности. Реальные API: obrnadzor.gov.ru (аккредитация, лицензии, контроль), vuz.minobrnauki.gov.ru (рейтинги).
...
| `vuzy_zapret_priema` | Список вузов с запретом приёма обучающихся |
| `statistika_kontrolya` | Статистика контрольной деятельности Рособрнадзора |
| `rezultaty_proverok` | Результаты проверок Рособрнадзора |
| `reestr_ekspertov` | Реестр экспертов Рособрнадзора |
...
### `sovfed` — Совет Федерации РФ (7 инструментов, 3 ресурса, 2 промпта)
...
| `poimennoe_golosovanie` | Результаты поимённого голосования Совета Федерации |
...
### `kaznacheistvo` — Федеральное казначейство (9 инструментов, 3 ресурса, 2 промпта)
...
| `ispolnenie_byudzheta` | Данные об исполнении бюджета (фильтрация по `razdel` / `podrazdel` бюджетной классификации) |
...
| `spisok_razdelov_byudzheta` | Справочник разделов бюджетной классификации РФ |
| `spisok_podrazdelov_byudzheta` | Справочник подразделов бюджетной классификации РФ |
| `byudzhetnaya_smeta` | Бюджетная смета по номеру |
...
### `rosprirodnadzor` — Росприроднадзор (9 инструментов, 3 ресурса, 2 промпта)
...
| `poisk_lesnogo_nadzora` | Поиск проверок лесного надзора |
...
### `mvd` — МВД России (8 инструментов, 3 ресурса, 2 промпта)

Статистика преступности, ДТП, розыск, наркотические преступления, государственный контроль, лицензирование. Реальные API: мвд.рф, data.gov.ru.

| Инструмент | Описание |
|------|----------|
| `spisok_naborov_dannykh` | Справочник наборов открытых данных МВД |
| `spisok_vidov_prestupleniy` | Справочник видов преступлений |
| `spisok_vidov_dtp` | Справочник видов ДТП |
| `spisok_federalnykh_okrugov` | Справочник федеральных округов |
| `statistika_prestupnosti` | Статистика преступности с данными МВД |
| `statistika_dtp` | Статистика ДТП с данными МВД |
| `rozysk_del` | Данные розыска МВД России |
| `narkotiki` | Данные о наркотических преступлениях |

**Ресурсы:** `data://istochniki`, `data://struktura`, `data://zakonodatelstvo`

**Промпты:** `analiz_prestupnosti`, `obzor_dorozhnoy_bezopasnosti`

**Авторизация:** не требуется

### `rostekhnadzor` — Ростехнадзор (7 инструментов, 3 ресурса, 2 промпта)

Промышленная безопасность, атомный надзор, лицензирование ОПО, инциденты, радиационная обстановка. Реальные API: rostechnadzor.gov.ru, data.gov.ru.

| Инструмент | Описание |
|------|----------|
| `spisok_vidov_nadzora` | Справочник видов надзора Ростехнадзора |
| `spisok_klassov_opasnosti` | Справочник классов опасности ОПО |
| `spisok_vidov_litsenziy` | Справочник видов лицензий Ростехнадзора |
| `spisok_vidov_intsidentov` | Справочник видов инцидентов |
| `poisk_intsidentov` | Поиск инцидентов и аварий на ОПО |
| `poisk_litsenziy` | Поиск лицензий Ростехнадзора |
| `reestr_opo` | Реестр опасных производственных объектов |

**Ресурсы:** `data://istochniki`, `data://struktura`, `data://zakonodatelstvo`

**Промпты:** `analiz_intsidenta`, `obzor_promyshlennoy_bezopasnosti`
