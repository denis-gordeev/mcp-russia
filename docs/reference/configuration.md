...
| `MCP_RUSSIA_HTTP_TIMEOUT` | `30.0` | HTTP-таймаут в секундах |
...
|----------|-----------|--------------------|
| `bm25` | Показывает только N наиболее релевантных инструментов, мета-инструменты остаются доступными всегда | Базовый режим |
| `none` | Публикует весь каталог инструментов без фильтрации | Отладка, ручная проверка, очень большой контекст |
| `rezhim_koda` | Экспериментальный режим обнаружения через `get_tags`, `search` и `GetSchemas` | Продвинутые сценарии и внутренние тесты |
...
- `429` с учётом заголовка `Retry-After`
- `5xx`, если источник временно недоступен
- таймаут, если API отвечает дольше `MCP_RUSSIA_HTTP_TIMEOUT`
