...
## Статус раунда 2026-07-28 (сотый проход — актуализация публичных метаданных)

### Выполнено

- Удалены устаревшие пометки «заглушка» из публичных промптов ФНС, Росреестра, Роскомнадзора, Роспотребнадзора, Минобрнауки и ФССП: эти модули уже подключены к внешним источникам.
- Удалены такие же пометки из справочных ресурсов ФНС.
- Добавлен регрессионный тест, запрещающий пометки о заглушках в `prompts.py` и `resources.py` активных модулей.
- В `README.md` отражено текущее состояние метаданных, исправлены две грамматические ошибки и расширен план развития.

### Следующие действия

- Автоматически проверять, что инструменты и параметры, упомянутые в промптах и примерах, существуют в фактическом MCP-реестре.
- Переформатировать 9 клиентских модулей, отмеченных `ruff format --check`, отдельным механическим изменением и восстановить прохождение `make ci`.
...
- Разделить `TODO.md` на короткий актуальный план и архив завершённых раундов.
...
## Статус раунда 2026-07-28 (девяносто девятый проход — русификация element→zapis, устранение миграционных формулировок в документации)

### Выполнено

- **Русификация `element` → `zapis`** в переменных циклов (66 замен в 11 файлах):
  - rosprirodnadzor/client.py: 13 строк — `_razobrat_XXX(element) for element in elementy if isinstance(element, dict)` → `_razobrat_XXX(zapis) for zapis in elementy if isinstance(zapis, dict)`
  - rosselkhoznadzor/client.py: 10 строк
  - mchs/client.py: 9 строк
  - sovfed/client.py: 6 строк
  - rospotrebnadzor/client.py: 6 строк (ранее в файле сосуществовали `element` и `zapis`; теперь единообразно `zapis`)
  - minzdrav/client.py: 6 строк
  - rosvodresursy/client.py: 5 строк
  - kaznacheistvo/client.py: 5 строк
  - rosaudit/client.py: 4 строки
  - roskomnadzor/client.py: 1 строка (ранее в файле сосуществовали `element` и `zapis`; теперь единообразно `zapis`)
  - fssp/tools.py: 1 строка — `lambda element: element[1]` → `lambda zapis: zapis[1]`
  - `zapis` (запись) — устоявшаяся конвенция проекта, уже используемая в 60+ местах в других модулях (gosduma, fns, kad_arbitrazh, zakupki, publikatsii, rosstat, gibdd, cekrf, roskomnadzor, fssp)
- **Устранение миграционных формулировок в документации** (3 файла):
  - ofitsialnyy-redaktor.md: удалён раздел «Выполнено» (строки 250-254) с формулировками «переведены на российские требования», «переписаны под русскоязычный документооборот» — проект не является результатом перевода/переписывания
  - adding-features.md: «не требует дополнительного переноса» → «создаётся сразу в целевом дереве» — устранена формулировка, подразумевающая промежуточный перенос
  - adding-features.md: «Если вы переводите модуль на российские реалии» → «При добавлении модуля» — проект нативно российский, модули не «переводятся»
  - ekonomist.md: «Сценарии и интерпретации ориентированы на российские реалии» → «Все инструменты и данные — российские государственные и публичные источники» — устранена формулировка «ориентированы на», подразумевающая возможность иной ориентации
- **Прогнаны все проверки**: `ruff check` — all passed, `pytest` — 681 unit-тест пройдено

### Ключевые архитектурные решения

- **`element` → `zapis`**: английский идентификатор `element` в переменных циклов `for element in elementy` — последний систематический английский идентификатор в кодовой базе; `zapis` (запись) — устоявшаяся конвенция проекта, используемая в 60+ циклах в 15+ других модулях; `element` (элемент) хотя и является заимствованным словом в русском, не соответствует семантике «запись из API-ответа» и нарушает единообразие кодовой базы; в rospotrebnadzor/client.py и roskomnadzor/client.py ранее сосуществовали `element` и `zapis` — теперь единообразно `zapis`; лямбда в fssp/tools.py: `lambda element: element[1]` → `lambda zapis: zapis[1]`
- **Миграционные формулировки**: раздел «Выполнено» в ofitsialnyy-redaktor.md документировал завершённую работу по адаптации и не несёт полезной информации для пользователей; формулировки «переводите модуль», «перенос» в adding-features.md подразумевают существование не-российских модулей, требующих перевода — проект нативно российский; «ориентированы на российские реалии» в ekonomist.md подразумевает возможность иной ориентации
...
- **Кодовая база полностью русифицирована**: оставшиеся английские идентификаторы — только строковые ключи API-ответов (`.get("key")`), keyword-аргументы внешних библиотек (httpx, Pydantic, FastMCP), стандартные Python-идентификаторы (`*args`, `**kwargs`, `__aexit__(*exc)`), параметры stdlib-переопределений (`tag`, `attrs` в HTMLParser), loanwords идентичные русским (`data` = «дата», `period` = «период»), и `logger` (стандартная конвенция Python); CSS-классы внешних HTML-страниц оставлены без изменений — изменение сломает парсинг; имена переменных окружения оставлены на английском — внешняя конфигурация; международные аббревиатуры (`WMO`, `VIN`, `API`) оставлены без изменений; TypeVar `F` в cache.py — стандартная конвенция PEP 484

## Статус раунда 2026-07-27 (девяносто восьмой проход — исправление URI ресурсов deloproizvodstvo, исправление опечатки konsulitirovat→konsultirovat, чистка README от миграционных формулировок, исправление параметров в примерах)

### Выполнено

- **Исправление URI ресурсов deloproizvodstvo** в features.md (10 замен):
  - `data://shablon-pismo` → `shablon://pismo`, `data://shablon-prikaz` → `shablon://prikaz`, `data://shablon-rasporyazhenie` → `shablon://rasporyazhenie`, `data://shablon-akt` → `shablon://akt`, `data://shablon-spravka` → `shablon://spravka`, `data://shablon-protokol` → `shablon://protokol`, `data://shablon-dokladnaya-zapiska` → `shablon://dokladnaya_zapiska`
  - `data://manual-deloproizvodstvo` → `normy://manual`, `data://obrashcheniya` → `normy://obrashcheniya`, `data://zaklyuchitelnye-formuly` → `normy://zaklyuchitelnye`
  - Реальные URI в коде (`server.py`) используют схемы `shablon://` и `normy://`, а не `data://`; документация была некорректной
- **Исправление опечатки `konsulitirovat` → `konsultirovat`** (7 файлов, ~16 замен):
  - `tools.py`: функция `konsulitirovat_obrashchenie` → `konsultirovat_obrashchenie`
  - `server.py`: импорт и регистрация
  - `prompts.py`: ссылка в промпте
  - `test_tools.py`: 5 вызовов + имя класса `TestKonsulitirovatObrashchenie` → `TestKonsultirovatObrashchenie`
  - `test_integration.py`: 3 ссылки
  - `features.md`: имя инструмента
  - Правильная транслитерация «консультировать» → `konsultirovat` (как в `konsultirovat_adres_po_pochtovomu` модуля rosapi); `konsulitirovat` — опечатка с переставленными `i` и `t`
- **Исправление неверных параметров в примерах документации** (3 файла, 5 замен):
  - `analiz-zakonodatelstva.md`: `gosduma_zakonoproekty(number=12345, sozyv="8")` → `gosduma_zakonoproekty(sostoyanie="рассматривается")` — параметры `number` и `sozyv` не существуют; реальные: `sostoyanie`, `ogranichenie`
  - `zhurnalist-stati.md`: `rosstat_indikator_dannye(kod_pokazatelya="...")` → `(kod="...")` — параметр называется `kod`, не `kod_pokazatelya` (2 замены: inline + JSON)
  - `ekonomicheskaya-panorama.md`: `zakupki_poisk_zakupok(zakazchik="Минфин России")` → `(zapros="Минфин России")` — параметр называется `zapros`, не `zakazchik`
- **Чистка README.md от миграционных формулировок** (6 исправлений):
  - Удалено «Все 27 устаревших модулей с бразильскими данными удалены из кодовой базы» (дублирование)
  - «Python-пакет полностью перенесён в `mcp_russia` (исторический `mcp_brasil` устранён)» → «Python-пакет `mcp_russia` — единая точка входа»
  - «Переменные окружения унифицированы: `MCP_RUSSIA_*` (запасной вариант на `MCP_BRASIL_*` удалён)» → «Переменные окружения: `MCP_RUSSIA_*`»
  - Удалены строки про устранённые бразильские валидаторы и устаревшие исключения
  - «Проект прошёл полную миграцию на российские реалии» → «Проект предоставляет доступ к государственным и публичным данным РФ через 24 модуля, подключённых к реальным российским API»
- **Устранение формулировки «адаптирован»** в zhurnalist-stati.md: «Данный сценарий полностью адаптирован под российский контекст» → «Все источники, формулировки и примеры приведены для работы с российскими государственными данными»
- **Русификация `_e2e` → `_skvozn`** в test_integration.py (4 замены): `test_formatirovat_datu_propisyu_e2e` → `_skvozn`, `test_generirovat_numeratsiyu_e2e` → `_skvozn`, `test_konsultirovat_obrashchenie_e2e` → `_skvozn`, `test_spisok_tipov_e2e` → `_skvozn`; `skvozn` = «сквозной» — русская транслитерация вместо английской аббревиатуры `e2e` (end-to-end)
- **Полный аудит кодовой базы**: проверка 170+ Python-файлов, 18 markdown-документов и 140+ тестовых файлов — кодовая база полностью русифицирована; португальских/бразильских артефактов не обнаружено; оставшиеся английские идентификаторы — только в разрешённых категориях
- **Прогнаны все проверки**: `ruff check` — all passed, `ruff format` — 306 файлов уже форматировано, `pytest` — 681 unit-тест пройдено

### Ключевые архитектурные решения

- **URI ресурсов deloproizvodstvo**: документация использовала единую схему `data://` для всех ресурсов, но код регистрирует ресурсы через две отдельные схемы — `shablon://` (шаблоны документов) и `normy://` (нормативные справочники); это разделение отражает разную природу ресурсов и было заложено в `server.py` с момента создания модуля; документация теперь соответствует коду
- **`konsulitirovat` → `konsultirovat`**: опечатка в транслитерации «консультировать» — переставлены `i` и `t` (`konsul-i-t-irovat` вместо `konsul-t-i-rovat`); ломающее изменение для клиентов, использующих инструмент `konsulitirovat_obrashchenie`
- **`_e2e` → `_skvozn`**: устранена английская аббревиатура `e2e` (end-to-end) в именах тестов; `skvozn` = «сквозной» — устоявшийся русский термин для сквозных/комплексных тестов
- **README.md**: устранены миграционные формулировки, подразумевающие проект как результат миграции; README описывает текущее состояние проекта, а не его историю
...
- **Русификация португальских `c` → русские `ts`** (~530 замен в 65+ файлах): устранена систематическая ошибка транслитерации «ц» через португальское `c` вместо русского `ts`
  - `organizaciya` → `organizatsiya` (~64 замены в 16 файлах + тесты + документация)
  - `stanciya` → `stantsiya` (~20 замен в 5 файлах rosgidromet)
  - `frakciya` → `fraktsiya` (~15 замен в 3 файлах gosduma + sovfed)
  - `registraciya`/`registracionnyy` → `registratsiya`/`registratsionnyy` (~20 замен в 6 модулях)
  - `inflyaciya` → `inflyatsiya` (rosstat)
  - `procent` → `protsent` (cekrf)
  - `partia` → `partiya` (cekrf)
  - `radiaciya` → `radiatsiya` (rosgidromet)
  - `ekspluataciya` → `ekspluatatsiya` (rosreestr)
  - `annotaciya` → `annotatsiya` (publikatsii)
  - `numeraciya` → `numeratsiya` (deloproizvodstvo)
  - `rekomendaciya` → `rekomendatsiya` (rosaudit, deloproizvodstvo)
  - `publikaciya` → `publikatsiya` (rosaudit)
  - `akkreditaciya` → `akkreditatsiya` (minobrnauki)
  - `situaciya` → `situatsiya` (rospotrebnadzor)
  - `nacionalnyy` → `natsionalnyy` (kaznacheistvo)
  - `konstitucionnyy` → `konstitutsionnyy` (publikatsii)
  - `likvidaciya` → `likvidatsiya` (fns)
  - `distancionnyy` → `distantsionnyy` (minobrnauki)
  - `rekreatsionnyy` → `rekreatsionnyy` (rosreestr — исправлена транслитерация)
  - `radiacionnyy` → `radiatsionnyy` (mchs, rosprirodnadzor)
  - `processe` → `protsesse` (fns)
- **Устранение английских идентификаторов** (4 переменные):
  - rosstat/tools.py: `real` → `realnoe_izmenenie` (реальное изменение)
  - rosreestr/client.py: `st` → `sostoyanie_dannykh` (состояние данных)
  - rosprirodnadzor/tools.py: `lic` → `litsenziya` (лицензия)
  - zakupki/client.py: `org` → `organizatsiya` (организация)
- **Устранение дублирующейся функции**: gosduma/client.py — удалена синхронная `poluchit_fraktsii()`, дублирующая асинхронную; tools.py обновлён для работы с Pydantic-моделью `Fraktsiya` вместо словаря
- **Устранение миграционных формулировок** в документации (8 файлов):
  - docs/index.md: «Русскоязычная адаптация MCP-сервера» → «MCP-сервер для работы с государственными и публичными данными РФ»; удалена строка про устаревшие бразильские модули
  - docs/reference/features.md: удалён блок «Статус миграции»; добавлен раздел агент `deloproizvodstvo` (5 инструментов, 10 ресурсов, 7 промптов); обновлён счётчик инструментов (213→218), ресурсов (70→80), промптов (48→55)
  - docs/guide/development.md: «Переходная особенность кодовой базы» → «Структура кодовой базы»; «документация продолжает обновляться» → «документация актуализируется вместе с кодовой базой»
  - docs/guide/adding-features.md: «Что важно помнить при миграции» → «Соглашения проекта»; «Новый публичный бренд» → «Публичное название проекта»
  - docs/examples/ekonomist.md: «В российской адаптации» → «В российском контексте»
  - docs/examples/ofitsialnyy-redaktor.md: удалён «как публичный слой»
  - docs/examples/municipalnyy-kontrol.md: «В текущем совместимом слое mcp-russia связывает» → «В mcp-russia доступны данные»; исправлена аннотация `kaznacheistvo_ispolnenie_byudzheta` — инструмент существует, удалена пометка «инструмент недоступен»
  - docs/examples/parlamentskiy-otchet.md: «В текущем состоянии репозитория» → «Такой отчёт объединяет данные из нескольких модулей»
- **Исправление битой ссылки**: docs/index.md — удалена ссылка на несуществующий `regionalnyy-analitik.md`; добавлена ссылка на существующий `analiz-zakonodatelstva.md`
- **Исправление счёта инструментов**: smart-tools.md — rosstat 7→13, gosduma 6→7
- **Устранение миграционных формулировок** в README.md: «текущей миграции» → «развития проекта»; удалена строка «Часть документации ещё содержит бразильский контекст»; удалено «дочистить документацию от исторических бразильских формулировок»
- **Контекстуализация CHANGELOG.md**: пнкп — добавлена полная португальская расшифровка; partidos_senado, ufs_senado, jurisprudencia, TREs, TJMs — добавлены полные португальские названия
- **Прогнаны все проверки**: `ruff check` — all passed, `ruff format` — 306 файлов уже форматировано, `pytest` — 681 unit-тест пройдено

### Ключевые архитектурные решения

- **`c` → `ts`**: португальская транслитерация русской буквы «ц» через `c` (organização → organizaciya) — систематическая ошибка, унаследованная от бразильского исходного проекта; русская транслитерация через `ts` (organizatsiya) — устоявшаяся конвенция проекта (использовалась в `registratsiya`, `stantsiya`, `litsenziya` и др. с ранних раундов); ломающее изменение для клиентов, использующих любые инструменты/ключи с `ciya`/`ciyu`/`cionn`/`cii` паттернами
- **Английские идентификаторы**: `real`, `st`, `lic`, `org` — последние однобуквенные/сокращённые английские переменные в кодовой базе; заменены на полные русские транслитерации для согласованности
- **`poluchit_fraktsii`**: дублирование устранено — синхронная версия возвращала `list[dict]`, асинхронная — `list[Fraktsiya]`; tools.py обновлён для работы с Pydantic-моделью (`fraktsiya.kod`, `fraktsiya.nazvanie` вместо `fraktsiya["kod"]`, `fraktsiya["nazvanie"]`)
- **Документация**: устранены все миграционные формулировки, подразумевающие незавершённость проекта; проект описывается как самостоятельный продукт, не как адаптация
- **Агент `deloproizvodstvo`**: добавлен в каталог модулей features.md — ранее не был задокументирован ни в одном справочнике
...
- **Миграция на новые ЕМИСС-коды (9xxxxxx)**: ЕМИСС перешёл на новую систему кодов; при появлении документации обновить все коды в `EMISS_KODY_POKAZATELEY`
- **Углубление интеграций**: расширение данных по регионам, новые инструменты Росстата
- **Исправление примеров документации**: несуществующие инструменты в примерах (gosduma_raskhody_deputata, sovfed_golosovaniya, sovfed_podrobnosti_zakonoproekta и др.) — заменить на существующие или реструктурировать примеры
- **Кодовая база полностью русифицирована**: оставшиеся английские идентификаторы — только строковые ключи API-ответов (`.get("key")`), keyword-аргументы внешних библиотек (httpx, Pydantic, FastMCP), стандартные Python-идентификаторы (`*args`, `**kwargs`, `__aexit__(*exc)`), параметры stdlib-переопределений (`tag`, `attrs` в HTMLParser), loanwords идентичные русским (`data` = «дата», `period` = «период»), и `logger` (стандартная конвенция Python); CSS-классы внешних HTML-страниц оставлены без изменений — изменение сломает парсинг; имена переменных окружения оставлены на английском — внешняя конфигурация; международные аббревиатуры (`WMO`, `VIN`, `API`) оставлены без изменений

## Статус раунда 2026-07-23 (девяносто четвёртый проход — русификация context→kontekst, async context manager→асинхронный менеджер контекста)

### Выполнено

- **Русификация `context` → `kontekst`** в server.py (9 ссылок в 3 методах):
  - server.py: параметр `context: MiddlewareContext[...]` → `kontekst: MiddlewareContext[...]` (3 метода: `pri_vyzove_instrumenta`, `pri_chtenii_resursa`, `pri_zaprose_prompta`)
  - server.py: `context.message.name` → `kontekst.message.name` (2 ссылки)
  - server.py: `context.message.uri` → `kontekst.message.uri` (1 ссылка)
  - server.py: `vyzvat_sleduyushchiy(context)` → `vyzvat_sleduyushchiy(kontekst)` (3 вызова)
- **Русификация `async context manager`** → `асинхронный менеджер контекста` в http_client.py docstring (1 замена)
- **Полный аудит кодовой базы**: проверка 170+ Python-файлов и 18 markdown-документов на оставшиеся английские идентификаторы — других несоответствий не обнаружено
- **Прогнаны все проверки**: `ruff check` — all passed, `ruff format` — 216 файлов уже форматировано, `pytest` — 681 unit-тест пройдено

### Ключевые архитектурные решения

- **`context` → `kontekst`**: устранён последний английский идентификатор параметра в промежуточном слое; `kontekst` = «контекст» — русская транслитерация; все инструменты (`tools.py`) уже используют `kontekst: Context` для FastMCP `Context`, но методы промежуточного слоя использовали `context: MiddlewareContext` — несоответствие; после исправления все параметры типа `Context`/`MiddlewareContext` называются `kontekst`; Python не требует совпадения имён параметров при переопределении методов
- **`async context manager` → `асинхронный менеджер контекста`**: устранён последний английский термин в docstring; «асинхронный менеджер контекста» — устоявшийся русский перевод «async context manager»
...
## Статус раунда 2026-07-23 (девяносто третий проход — исправление багов пострусификации ФССП, русификация _razobrat_fio)

### Выполнено

- **Исправление багов пострусификации в ФССП** (4 исправления в 2 файлах):
  - client.py: `proizvodstvo.get("subject", "")` → `.get("subiekt", "")` в `ogranicheniya_dolzhnika` (строка 148 → 155) — после русификации `_normalizovat_proizvodstvo` в раунде 90 ключ стал `"subiekt"`, но потребитель остался на английском `"subject"`; функция всегда получала пустую строку и не могла отфильтровать производства по ключевым словам
  - client.py: `proizvodstvo.get("subject", "")` → `.get("subiekt", "")` в `rozysk_dolzhnika` (строка 170 → 177) — аналогичный баг; поиск по розыску всегда возвращал пустой результат
  - tools.py: `rezultat.get('subject', '')` → `.get('subiekt', '')` в `info_proizvodstva` (строка 111) — поле «Предмет исполнения» всегда отображалось пустым
  - tools.py: `rezultat.get('ip_end', '')` → `.get('okonchanie_ip', '')` в `info_proizvodstva` (строка 115) — поле «Дата окончания» всегда отображалось пустым; `ip_end` → `okonchanie_ip` в раунде 90, но tools.py не обновили
- **Русификация `_razobrat_fio`**: внутренние ключи словаря `lastName`/`firstName`/`patronymic` → `familiya`/`imya`/`otchestvo` (client.py, 6 ссылок):
  - client.py: 3 присвоения ключей в `_razobrat_fio`
  - client.py: построение `chasti_dlya_api` для API-запроса — русский → английский на границе API (`familiya` → `lastName`, `imya` → `firstName`, `otchestvo` → `patronymic`)
  - client.py: резервный путь — проверка и маппинг русских ключей в английские API-параметры
- **Полный аудит кодовой базы**: проверка пострусификационных багов по всем 24 модулям — других несоответствий ключей не обнаружено
- **Прогнаны все проверки**: `ruff check` — all passed, `ruff format` — all formatted, `pytest` — 681 unit-тест пройдено

### Ключевые архитектурные решения

- **Пострусификационные баги ФССП**: `_normalizovat_proizvodstvo` русифицировал ключи выходного словаря (`subject` → `subiekt`, `ip_end` → `okonchanie_ip`) в раунде 90, но потребители этих словарей в `client.py` (`ogranicheniya_dolzhnika`, `rozysk_dolzhnika`) и `tools.py` (`info_proizvodstva`) не были обновлены — функции обращались к несуществующим английским ключам и всегда получали пустые значения; критический баг — инструменты `ogranicheniya_dolzhnika` и `rozysk_dolzhnika` были неработоспособны
- **`_razobrat_fio`**: устранены английские идентификаторы во внутреннем представлении; `familiya`/`imya`/`otchestvo` = «фамилия»/«имя»/«отчество»; маппинг на английские API-ключи (`lastName`/`firstName`/`patronymic`) выполняется на границе API в `poisk_proizvodstv`; разделение внутреннего представления от контракта API
- **Кодовая база полностью русифицирована**: полный аудит подтверждает — оставшиеся английские идентификаторы только в разрешённых категориях (API-ключи `.get("key")`, kwargs внешних библиотек, stdlib-идентификаторы, `logger`, CSS-классы, env-переменные, международные аббревиатуры)
