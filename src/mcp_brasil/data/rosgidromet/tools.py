...
async def pogoda_seychas(stanciya: str = "77", ctx: Context | None = None) -> str:
    ...
    if data.temperatura:
        lines.append(f"- Температура: {format_number_br(data.temperatura, 1)}°C")
    if data.feels_like:
        lines.append(f"- Ощущается как: {format_number_br(data.feels_like, 1)}°C")
    if data.vlazhnost:
        lines.append(f"- Влажность: {format_number_br(data.vlazhnost, 0)}%")
    if data.davlenie:
        lines.append(f"- Давление: {format_number_br(data.davlenie, 0)} мм рт.ст.")
    if data.veter_skorost:
        lines.append(
            f"- Ветер: {format_number_br(data.veter_skorost, 1)} м/с {data.veter_napravlenie}"
        )
    if data.osadki is not None:
        lines.append(f"- Осадки: {format_number_br(data.osadki, 1)} мм")
    if data.opisaniye:
...
async def prognoz_pogody(
    ...
    for p in prognoz:
        ...
        if p.temperatura_dnem is not None:
            lines.append(f"- Днём: {format_number_br(p.temperatura_dnem, 1)}°C")
        if p.temperatura_nochyu is not None:
            lines.append(f"- Ночью: {format_number_br(p.temperatura_nochyu, 1)}°C")
        if p.osadki_veroyatnost is not None:
            lines.append(f"- Осадки: {format_number_br(p.osadki_veroyatnost, 0)}%")
        if p.opisaniye:
...
async def ekologiya_regiona(
    ...
    for d in data[:10]:
        ...
        if d.znachenie is not None:
            line += f" = {format_number_br(d.znachenie, 2)}"
        if d.prevyshenie:
