...
Для добавления новой функции достаточно создать директорию по конвенции CONTRIBUTING.md.
...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "26 модулей данных подключены к реальным API: ЦБ РФ, "
        "Росгидромет (через Open-Meteo), ФНС, Госдума, Закупки, "
        "КАД Арбитраж, РосАПИ (Dadata), Росреестр, ГИБДД, ЦИК РФ, ФССП, "
        "Официальные публикации, Минобрнауки/Рособрнадзор, "
        "Роспотребнадзор, Роскомнадзор, Росстат, Росводресурсы, "
        "Минздрав, Счётная палата, Совет Федерации, Казначейство, "
        "Росприроднадзор, МЧС, Россельхознадзор, МВД, Ростехнадзор. "
        "1 агентный модуль: Делопроизводство."
    ),
    version="0.6.0",
