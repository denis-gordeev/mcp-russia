...
├── _shared/            # Общий код (http-клиент, форматирование, кеш, ограничитель частоты, обнаружение, планировщик, пакетное выполнение)
├── data/               # Модули для внешних API (26 модулей данных)
│   ├── cbrf/           # Центральный банк РФ
│   ├── cekrf/          # ЦИК РФ
│   ├── fns/            # ФНС
│   ├── fssp/           # ФССП
│   ├── gibdd/          # ГИБДД
│   ├── gosduma/        # Государственная Дума
...
├── resources.py    # MCP-ресурсы (при необходимости)
└── prompts.py      # MCP-промпты (при необходимости)
```
...
- **Непрерывная интеграция** (`.github/workflows/ci.yml`): запускается при каждом отправлении изменений/пул-реквесте в `main` — линтер + типы + тесты (Python 3.10-3.13)
- **Релиз** (`.github/workflows/release.yml`): запускается по тегу `v*` — CI + сборка + публикация в PyPI + релиз на GitHub
...
- Если обнаружили технический долг или следующий шаг развития, зафиксируйте его в `TODO.md`
