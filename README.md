...
Русскоязычный MCP-сервер для государственных и публичных данных РФ.
...
- Все модули передают `instructions` (описание) и `version` (версия) в MCP-протокол через конструктор `FastMCP()`.
## Что здесь есть
...
3. При добавлении модуля обновляйте одновременно код, тесты и документацию.
