...
Python-пакет `mcp_russia` является единой точкой входа. Историческое имя `mcp_brasil` полностью устранено из кодовой базы, импортов, конфигурации и переменных окружения. Все 27 legacy-модулей с бразильскими данными отключены (`enabled=False`) и не загружаются в сервер.
...
- при необходимости полностью удалить отключённые legacy-модули.
...
Часть документации и legacy-модулей ещё содержит бразильский контекст, но эти модули отключены и не загружаются. Рабочая точка входа для запуска и разработки — `mcp_russia`.
...
Актуализируйте существующую документацию с учётом фактической структуры `src/mcp_russia/`. Часть файлов в `docs/` и `CHANGELOG.md` содержит исторические бразильские формулировки и требует отдельного прохода. Legacy-модули отключены и не влияют на работу сервера.
...
Проект прошёл содержательную миграцию на российские реалии. 22 модуля активны и подключены к реальным российским API. 27 legacy-модулей с бразильскими данными отключены и не загружаются.
