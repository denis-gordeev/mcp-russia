...
Python-пакет `mcp_russia` является единой точкой входа. 26 российских модулей данных + 1 агентный модуль активны и подключены к реальным API. Сервер экспонирует 257 русскоязычных инструментов, 90 ресурсов и 62 промпта.
...
- Все 26 клиентских модулей проходят строгую типизацию `mypy`: безопасная нормализация внешних ответов, сужение объединённых типов, явные проверки `None`.
...
Проект предоставляет доступ к государственным и публичным данным РФ через 26 модулей данных и 1 агентный модуль, подключённых к реальным российским API.
