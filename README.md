...
Python-пакет `mcp_russia` является единой точкой входа. 26 российских модулей данных + 1 агентный модуль активны и подключены к реальным API. Сервер экспонирует только русскоязычные инструменты и ресурсы.
